module Responses
  class PhotosResponse
    ...
    def self.respond(photos, opts = EMPTY_HASH)
      ...
      foursquare = (photos[:foursquare] || []).group_by(&:prefix).values.collect do |o|
        by_width = o.sort_by(&:width)
        by_width.last.as_json.merge(small_url: by_width.first.url)
      end

      yelp = photos[:yelp].collect {|o| o.as_json.merge(small_url: o.url) }.uniq {|o| o[:url] } if photos[:yelp]

      [yelp, google, facebook, foursquare].compact.flatten.take(count.to_i)
    end
  end
end
