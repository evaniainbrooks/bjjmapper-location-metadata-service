...
class FacebookPhoto 
  ...
  COLLECTION_FIELDS = [:coordinates, :is_cover_photo, :is_profile_photo, :photo_id,
  ...
  def as_json
    (COLLECTION_FIELDS - [:_id, :source, :coordinates]).inject({}) do |hash, k|
      hash[k] = self.send(k)
      hash
    end.merge(key: key, source: 'Facebook', url: self.source, lat: lat, lng: lng)
  end
end
